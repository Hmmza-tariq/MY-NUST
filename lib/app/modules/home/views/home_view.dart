                    ...
                    child: Row(
                      children: [
                        InkWell(
                            onTap: () {
                              launchUrl(Uri.parse(
                                'https://maps.app.goo.gl/5WVuv7pCzZDz8zU1A',
                              ));
                            },
                            child: Image.asset(AssetsManager.logo, width: 32)),
                        const Spacer(),
                        Obx(() => CustomButton(
                              onPressed: () => Get.defaultDialog(
                                title: 'Select Campus',
                                content: SizedBox(
                                  height: controller.campuses.length / 3 * (80),
                                  width: Get.width * 0.9,
                                  child: GridView(
                                    gridDelegate:
                                    ...
                                    children: [
                                      ...controller.campuses.map((campus) =>
                                          CustomButton(
                                              isBold: false,
                                              title: campus,
                                              onPressed: () {
                                                controller.selectedCampus
                                                    .value = campus;
                                                Get.back();
                                              },
                                              color: campus ==
                    ...
                    return SizedBox(
                      ...
                      child: CarouselSlider.builder(
                        ...
                        itemBuilder: (context, index, realIndex) {
                          if (isLoading) {
                            return Center(
                              child: buildLoadingContainer(index, activePage),
                            );
                          }

                          final story = topStories[index];
                          return buildStoryContainer(story, index, activePage);
                        },
                      ),
                    );
                  }),
